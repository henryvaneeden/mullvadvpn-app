    ...
    return (
      <ModalAlert
        ...
        <ModalMessage>
          {messages.pgettext(
            'advanced-settings-view',
            'WARNING: This is not the app\'s "kill switch". The kill switch is always enabled and has no setting. Enabling this means blocking all network traffic when you have disabled the VPN. The app always protects against leaks while connecting, changing server and during unexpected tunnel failures without this setting.',
          )}
        </ModalMessage>
        <ModalMessage>
          {messages.pgettext(
            'advanced-settings-view',
            'When this setting is enabled, the app blocks all network traffic when disconnected, even after quitting the app.',
          )}
        </ModalMessage>
      </ModalAlert>
    );
